				...
				if (ev.elements) {
					for (var i = 0; i < ev.elements.length; i++) {
						...
						if (el.id) {
							...
							if (ms.mouse_on_select === false) div.click(); // just a workaround solution, the mouse position gets lost
																		   // when the mouse is over a select field, triggering weird behaviour otherwise 
							console.log(el.id);
