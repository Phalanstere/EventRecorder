...
function EventRecorder(params) {
	...
	this.display = function display() {
		...
		switch(ev.type) {
			...
			case "scroll":
				el = document.getElementById(ev.element_id);
				el.scrollTop = ev.scrollTop;
			break;
	...
	this.get_scroll_position = function get_scroll_position() {
		...
		if (! ms.scrolls) { 
			ms.scrolls = document.getElementsByTagName("*");
			for (i = 0; i < ms.scrolls.length; i++) {
				ms.scrolls[i].sl = ms.scrolls[i].scrollLeft;
				ms.scrolls[i].st = ms.scrolls[i].scrollTop;
			...
			for (i = 0; i < ms.scrolls.length; i++) {
				...
				if (el.st !== el.scrollTop) {
					if (el.id) {
						time = new Date().getTime() - ms.start;
						console.log("HALLO " + el.id);
	...
	this.init = function init() {
		...
		document.onmousewheel	= ms.scroll;
