...
function EventRecorder(params) {
	...
	//
	this.key_input = function key_input(key) {
		var c,
		text, 
		...
		if (key.keyCode === 8) // Backspace
			...
			{
			c = String.fromCharCode(key.charCode);
			text += c;
	...
	this.special_keys = function special_keys(keyCode) {
	...
	this.display = function display() {
		...
		switch(ev.type) {
			...
			case "keypress":
				if (ev.key_ctrl) alert("KONTROLLE");
				ms.key_input(ev.keyCode);
			break;
			
			case "keyup":
			   ms.special_keys(ev.keyCode);
			break;
	...
	this.check_onmouseover = function check_onmouseover(event) {
		var old_elements = ms.touched_elements;
		ms.touched_elements = ms.on_element(event);
		
		var x = ms.touched_elements.diff( old_elements);
		
		if (x.length !== 0) console.log(x.length);
	...
	this.keydown = function keydwon(event) {
		switch(event.keyCode)
			{
			case 17:
			  console.log("KEYCONTROL_TRUE");
			  ms.key_ctrl = true;
			break;	
